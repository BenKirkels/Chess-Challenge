...
// MyBot3-3
// Replace simplefied evaluation function
        ...
        foreach (bool whitePiece in new[] { true, false })
        {
            for (int piece = 1; piece <= 6; piece++)
            {
                ...
                while (bitBoard != 0)
                {
                    ...
                    int index = 128 * (piece - 1) + BitboardHelper.ClearAndGetIndexOfLSB(ref bitBoard) ^ (whitePiece ? 56 : 0);
                    mg += GetPieceSquareValue(index) + pieceEval[piece];
                    eg += GetPieceSquareValue(index + 64) + pieceEval[piece];
                }
            }
            mg = -mg;
            eg = -eg;
