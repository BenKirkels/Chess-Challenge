...
// Defeats the EvilBot (almost) every time
// Plays better than chess.com level 1200 but loses on time

public class MyBot : IChessBot
{
    readonly int searchDepth = 3;
    public Move Think(Board board, Timer timer)
    {
        ...
        int BestEval = IAmWhite ? -100000 : 100000;

        foreach (Move move in moves)
        {
            board.MakeMove(move);
            int eval = Minimax(board, searchDepth, -100000, 100000, !IAmWhite);
            board.UndoMove(move);
            if (IAmWhite)
            {
                if (eval > BestEval)
